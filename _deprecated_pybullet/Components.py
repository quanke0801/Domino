...
class SingleDomino(Component):
	...
	def Create(self, base_pose = ([0, 0, 0], [0, 0, 0, 1])):
		if self.created:
			return
		if SingleDomino.SHARED_COLLISION_SHAPE is None:
			SingleDomino.SHARED_COLLISION_SHAPE = createCollisionShape(
				GEOM_BOX,
				halfExtents = [S / 2 for S in DOMINO_SIZE])
		now_pose = multiplyTransforms(
			base_pose[0], base_pose[1], self.pose[0], self.pose[1])
		self.body_id = createMultiBody(
			DOMINO_MASS,
			SingleDomino.SHARED_COLLISION_SHAPE,
...
def GenerateCrossingProfile(contact_ratio):
	alpha = fsolve(lambda x: (SZ - SX / math.sin(x[0])) * math.cos(x[0]) + contact_ratio * SX - SY, [math.asin(SX / SZ)])[0]
