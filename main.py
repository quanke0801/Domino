...
def build_scene_2() -> Component:
    ...
    scene.add_child("support", (
        Domino()
        .orient(Domino.lying(np.pi / 2))
        .place_abs("x+", np.array([0, 0, 0]))
    ))
    scene.add_child("output", (
        Domino()
        .orient(Domino.standing())
        .place_snap("x+z-", scene.child("support").anchor("x-y-"))
        .move(np.array([0.0072, 0, 0]))
    ))
    scene.add_child("shaft_slanted", (
        Domino()
        .orient(Domino.lying())
        .place_snap("x+z+", scene.child("output").anchor("x-z-"))
        .move(np.array([0.00092, 0, 0]))
        .rotate_to_touch(np.array([0.0075, 0, 0.015]), np.array([0, -1, 0]), None)
    ))
